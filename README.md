...
<img src="docs/install-qr.png" width="200" alt="QR code linking to the latest DSA Mastery APK download">
<br><sub><b>Scan to install</b></sub>
</td>
<td>

**[⬇ Download the APK directly](https://github.com/imsky1812/dsaTracker/releases/latest/download/DSA-Mastery.apk)**

or see all versions on the **[releases page](https://github.com/imsky1812/dsaTracker/releases)**
on your phone.
...
> The download link always serves the APK from the newest
> [GitHub release](https://github.com/imsky1812/dsaTracker/releases), so it
> never expires (EAS's own artifact links are deleted after two weeks). To cut a
...
After a build, publish the APK as a GitHub release. EAS artifact links expire
after two weeks; a release asset does not:

```bash
gh release create v<version> path/to/DSA-Mastery.apk --title "DSA Mastery <version>"
```

Keep the asset named `DSA-Mastery.apk`. The README's download button and QR
point at `releases/latest/download/DSA-Mastery.apk`, which always resolves to
the newest release, so neither needs changing per build. The QR is generated by:

```bash
npm run qr -- https://github.com/imsky1812/dsaTracker/releases/latest/download/DSA-Mastery.apk
```
